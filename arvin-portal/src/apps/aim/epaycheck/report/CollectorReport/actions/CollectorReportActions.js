...
export const getCollectorReportData = (formValues) => async (dispatch) => {
  try {
    dispatch({
    ...
    const response = await GetSpecificDefaultServices(
      `api/epaycheck/check_detail_logs/get_collector_report_data?df=${formValues.df}&dt=${formValues.dt}&sap=${formValues.sap}`
    );
