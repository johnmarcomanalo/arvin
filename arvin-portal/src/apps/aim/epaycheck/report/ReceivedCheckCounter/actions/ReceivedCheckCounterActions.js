...
export const getReceivedCheckCounterData = (formValues) => async (dispatch) => {
  try {
    dispatch({
    ...
    const response = await GetSpecificDefaultServices(
      `api/epaycheck/report/get_received_check_counter?df=${formValues.df}&dt=${formValues.dt}&sc=${formValues.sc}`
    );
